...
#[test]
fn prepare() {
    let mut client = Client::connect("host=localhost port=5432 user=postgres", NoTls).unwrap();
...
fn transaction_drop_immediate_rollback() {
    let mut client = Client::connect("host=localhost port=5432 user=postgres", NoTls).unwrap();
    let mut client2 = Client::connect("host=localhost port=5432 user=postgres", NoTls).unwrap();
...
fn notice_callback() {
    ...
    let mut client = Config::from_str("host=localhost port=5432 user=postgres")
        .unwrap()
...
#[test]
fn explicit_close() {
    let client = Client::connect("host=localhost port=5432 user=postgres", NoTls).unwrap();
    client.close().unwrap();
}
